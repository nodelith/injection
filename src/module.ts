...
export class Module {
  ...
  public registerFactory(token: Token, factory: TargetFactory, options: RegistrationOptions): void {
    const resolver = Resolver.create({ factory })
    this.setRegistration(token, resolver, options)
  }

  public registerConstructor(token: Token, constructor: TargetConstructor, options: RegistrationOptions): void {
    const resolver = Resolver.create({ constructor })
    this.setRegistration(token, resolver, options)
  }

  private setRegistration<R extends object>(token: Token, resolver: Resolver<R>, options?: RegistrationOptions): void {
